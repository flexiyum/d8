...
cd
wget -O /usr/bin/updatescript.sh $source/master/updatescript.sh && clear && chmod +x /usr/bin/updatescript.sh && /usr/bin/updatescript.sh && rm -f /root/IP && rm -f /root/IPcarding
}
function anti_ddos() {
...
do
	...
	"Install SQLmap")
	cd /root
	wget -O /usr/bin/install-sqlmap.sh $source/master/install-sqlmap.sh
